...
using CrmCore.Security;
using CrmCore.Serialization;
...
namespace CrmCore
{
    ...
    public class SalesforceClient
    {
        ...
        /// <param name = "altUrl" > The url to use without the instance url</param>
        /// <returns>The API result for the query.</returns>
        public List<Record> GetRecipientEmail<T>(string query, string altUrl = "") where T : new()
        {
            ...
            dynamic returns = new List<T>();

            dynamic response = Request<SalesforceQueryResult<T>>(url);

            List<dynamic> records = JsonConvert.DeserializeObject(response.Content)["records"].ToObject<List<dynamic>>();

            List<Record> r = new List<Record>();

            foreach (dynamic record in records)
            {
                r.Add(new Record { ContactId = record["Id"], FirstName = record["FirstName"], PreferredEmail = record["Preferred"], AlternativeEmail = record["Alternate"] });
            }
        ...
        /// </summary>
        /// <param name="altUrl">The url to use without the instance url</param>
        /// <param name="reportId">The ID of the report to acquire contactIds for</param>
        /// <returns>The API result for the query.</returns>
        public int GetContactIdIndex<T>(string altUrl = "", string reportId = "") where T : new()
        {
            int idx = 0;
            var url = GetAltUrl(altUrl + reportId);

            dynamic response = Request<SalesforceQueryResult<T>>(url);

            List<string> fields = JsonConvert.DeserializeObject(response.Content)["reportMetadata"]["detailColumns"].ToObject<List<string>>();

            for (int i = 0; i < fields.Count(); i++)
            {
                if (fields[i] == "FK_CNTC_ID")
                {
                    idx = i;
                }
            }
        ...
        /// Executes a SOQL query and returns the result.
        /// </summary>
        /// <param name="idx">Column index of contact field</param>
        /// <param name="altUrl">The url to use without the instance url</param>
        /// <param name="reportId">The ID of the report to acquire contactIds for</param>
        /// <returns>The API result for the query.</returns>
        public List<string> GetContactIdsByReportId<T>(string altUrl = "", string reportId = "", int idx = 0) where T : new()
        {
            var url = GetAltUrl(altUrl + reportId);
            ...
            List<object> ids = JsonConvert.DeserializeObject(response.Content)["factMap"]["T!T"]["rows"].ToObject<List<object>>();

            foreach (JObject id in ids)
            {
                contactIds.Add(id["dataCells"][idx]["value"].ToString());
            }
        ...
        /// <param name="reportId">The ID of the report to acquire contactIds for</param>
        /// <returns>The API result for the query.</returns>
        public List<object> GetMergeObjectIdsByReportId<T>(string altUrl = "", string reportId = "") where T : new()
        {
            var url = GetAltUrl(altUrl + reportId);
            ...
            List<object> objs = JsonConvert.DeserializeObject(response.Content)["factMap"]["T!T"]["rows"].ToObject<List<object>>();
        ...
        public IList<T> QueryActionBatch<T>(string query, Action<IList<T>> action, string altUrl = "") where T : new()
        {
            ...
            } while (response != null && response.Data != null && !response.Data.Done && !string.IsNullOrEmpty(response.Data.NextRecordsUrl.ToString()));
        ...
        private string GetNextRecordsUrl<T>(IRestResponse<SalesforceQueryResult<T>> previousResponse) where T : new()
        {
            if (previousResponse == null || previousResponse.Data == null ||
                string.IsNullOrEmpty(previousResponse.Data.NextRecordsUrl.ToString()))
            {
                return string.Empty;
            }
        ...
        public T FindById<T>(string objectName, string recordId, string altUrl) where T : new()
        {
            ...
            var result = Query<T>("SELECT {0} FROM {1} WHERE Id = '{2}'".With(GetRecordProjection(typeof(T)), objectName, recordId), altUrl + recordId);
        ...
        /// <exception cref="InvalidOperationException">Please, execute Authenticate method before call any REST API operation.</exception>
    ...
    public class reportMetadata
    {
        ...
        /// 
        /// </summary>
        [Display(Name ="detailColumns")]
        public List<string> DetailColumns = new List<string>() { "Contact.Id", "Contact.Email" };
        /// <summary>
        /// 
        /// </summary>
        [Display(Name = "hasDetailRows")]
        public bool hasDetailRows { get; set; } = true;
        /// <summary>
        /// 
        /// </summary>
        [Display(Name = "hasRecordCount")]
        public bool hasRecordCount { get; set; } = true;
        /// <summary>
        /// 
        /// </summary>
        [Display(Name = "reportBooleanFilter")]
        public bool? reportBooleanFilter { get; set; } = null;
    }
